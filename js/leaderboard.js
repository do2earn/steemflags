...
function renderLeaderboard(accounts){
  ...
  body.innerHTML=accounts.map((a,i)=>{const u=String(a.Username||'—'),d=Math.max(0,Number(a.D2E)||0),flag=String(a.Flag||'').trim(),flagCode=FLAG_CODES.get(flag),medal=i===0?'🥇':i===1?'🥈':i===2?'🥉':String(i+1);const flagImage=flagCode?`<img class="leaderboardFictionalFlag" src="https://raw.githubusercontent.com/hampusborgos/country-flags/master/svg/${flagCode}.svg" alt="${escapeHtml(flag)} flag" loading="lazy" onerror="this.style.display='none'">`:!flag?'<img class="leaderboardFictionalFlag" src="assets/steem_fictional_flag.png" alt="" aria-hidden="true">':'';return `<tr><td>${medal}</td><td><div class="leaderboardUser">${flagImage}<img class="leaderboardAvatar" src="${avatarUrl(u)}" alt="@${escapeHtml(u)}" loading="lazy" onerror="this.style.visibility='hidden'"><span>@${escapeHtml(u)}</span></div></td><td>${d.toLocaleString()}</td></tr>`}).join('')||'<tr><td colspan="3" class="muted">No players yet.</td></tr>';
}
